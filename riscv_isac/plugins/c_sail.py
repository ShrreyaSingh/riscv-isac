...
class c_sail(spec.ParserSpec):
    ...
    instr_pattern_c_sail_csr_reg_val = re.compile('(?P<CSR>[R])\s(?P<reg>[a-z0-9]+)\s<-\s(?P<val>[0-9xABCDEF]+)')
    ...
    def extractCsrCommitVal(self, line):
        ...
        re_search = instr_pattern.search(line)
        if re_search is not None:
                return (re_search.group('CSR'), re_search.group('reg'), re_search.group('val'))
        else:
            return None
    ...
    def __iter__(self):
        ...
        for line in instructions:
            ...
            yield instr, mnemonic, addr, reg_commit, csr_commit
